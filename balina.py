...
# --- GÜVENLİ VERİ ÇEKME (ÇÖKME ÖNLEYİCİ) ---
def get_live_rates():
    try:
        ...
        if not data.empty:
            ...
            ga = (last.get('GC=F', 2600) * usd) / 31.1035
            gg = (last.get('SI=F', 30) * usd) / 31.1035
...
def get_market_analysis():
    # Listeyi kısa tutalım ki hızlı açılsın
...
def ana_uygulama(kalan_sure_dk=None):
    ...
    if st.session_state.secilen_hisse:
        res = grafik_ciz(st.session_state.secilen_hisse)
        if res and res[0]:
